...
3. Crea `features/<name>.feature` con:
   ...
   - Un `Scenario:` por comportamiento observable, incluyendo **casos
     límite y errores** (validación fallida, estado vacío, datos corruptos).
   - Pasos `Given` / `When` / `Then` concretos y verificables. Cada `Then`
     afirma algo medible: texto en pantalla, clase CSS, estado del componente.
...
- ✅ Nada de pasos vagos ("el componente funciona"). Cada paso es ejecutable.
