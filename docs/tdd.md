...
- **ROJO** — el test deriva del siguiente escenario `@s` del `.feature`.
   Verifícalo fallando de verdad (`npm test`). Un test que pasa a la
   primera no demuestra nada: ajústalo o sospecha del montaje.
...
Refactor. Un escenario con varias aristas (p. ej. "lista vacía muestra mensaje"
y "tres tareas muestra la lista") puede necesitar dos ciclos para forzar la
...
- @s2 (agregar tarea → aparece) → test_adds_todo_to_list
